...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="nogui",
        help="Sets the program to open a file/directory dialog to select files/directories instead of passing them as command line args"
    )
    parser.add_argument(
        "-out",
        nargs="?",
    ...
    parser.add_argument("-dir", nargs="?", metavar="directory", dest="dir", const=desktop_path, default=desktop_path, help="The directory to store the file in")
    parser.add_argument("-pdfs", nargs="*", help="The pdfs you want to merge. Omitted when --gui is used.")
    ...
    out_name = args.out if args.out.endswith(".pdf") else f"{args.out}.pdf" # Append pdf extension to outputfile name if not there
    try:
        for pdf in files:
            merger.append(pdf)
    except:
